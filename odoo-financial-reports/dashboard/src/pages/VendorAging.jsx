import { useVendorAging } from '../hooks/useReports';
...
export default function VendorAging() {
  ...
  const { data, isLoading, error } = useVendorAging(agingFilters);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <KPICard title={t('aging.overdue_total')} value={formatCompact(data?.overdue_total)} icon={AlertCircle}
          color={(data?.overdue_total ?? 0) > 0 ? 'amber' : 'emerald'} loading={isLoading} />
      ...
      {(isLoading || partners.length > 0) && (
        <DataTable columns={COLUMNS} data={partners} loading={isLoading} filename={`vendor-aging-${asOf}.csv`} />
      )}
    </div>
  );
}
