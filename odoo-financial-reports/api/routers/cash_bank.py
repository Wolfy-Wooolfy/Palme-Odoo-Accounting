...
from api.services.cash_bank import compute_cash_bank

router = APIRouter(prefix="/reports", tags=["reports"])


@router.post("/cash-bank")
def cash_bank(filters: ReportFilter, client=Depends(get_odoo_client)):
    cache_key = f"cash_bank:{filters.model_dump_json()}"
    ...
    try:
        data = compute_cash_bank(client, filters)
