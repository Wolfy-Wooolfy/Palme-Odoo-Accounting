...
from api.services.profit_loss import compute_profit_loss

router = APIRouter(prefix="/reports", tags=["reports"])


@router.post("/profit-loss")
def profit_loss(filters: ReportFilter, client=Depends(get_odoo_client)):
    cache_key = f"profit_loss:{filters.model_dump_json()}"
    ...
    try:
        data = compute_profit_loss(client, filters)
